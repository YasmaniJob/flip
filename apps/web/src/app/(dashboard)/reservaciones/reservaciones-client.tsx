...
export function ReservacionesClient() {
    ...
    const handleCellClick = useCallback((date: Date, hour: { id: string; isBreak?: boolean }) => {
        ...
        if (existingSlot) {
            const isPIPReservation = existingSlot.staff?.role === 'pip';
            const isWorkshop = existingSlot.type === 'workshop';
        ...
        <Suspense fallback={<LoadingSkeleton />}>
            <>
                ...
                <div className="hidden lg:block p-6 sm:p-8 max-w-[1600px] mx-auto min-h-screen space-y-6 relative">
                    <PageHeader title="Reservaciones del AIP" />
                    ...
                    {selectedReservationId && (
                        <Dialog open={!!selectedReservationId} onOpenChange={(open) => !open && setSelectedReservationId(null)}>
                            <DialogContent className="hidden lg:flex sm:max-w-2xl w-full h-full p-0 flex-col gap-0 border-none rounded-none fixed right-0 top-0 bottom-0 left-auto translate-x-0 translate-y-0 m-0 z-50 [&>button]:hidden">
                                ...
                                <WorkshopDetailSheet 
                                    ...
                                    onReschedule={() => {
                                        // Buscar el primer slot que coincida con esta reservación para el diálogo
                                        const slotToReschedule = slots?.find(s => s.reservationMainId === selectedReservationId);
                                        if (slotToReschedule) {
                                          setMobileSheetSlot(slotToReschedule);

...
// GET /api/institutions/provincias - Get provinces for a department (PUBLIC)
export async function GET(request: NextRequest) {
  try {
    ...
    // Cache provincias per departamento for 1 hour
    const getProvincias = unstable_cache(
      ...
      { revalidate: 3600, tags: ['provincias', `provincias-${departamento}`] } // 1 hour
    );
